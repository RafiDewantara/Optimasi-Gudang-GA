...
item_desc = df.groupby("ITEM CODE")["ITEM DESCRIPTION"].agg(
...
def decode_chromosome(chromosome):
    """
    chromosome: list of item indices (0..TOP_N-1) dalam urutan penempatan
    gen_pos 0 -> slot terdekat, gen_pos 1 -> slot berikutnya, dst.
    """
    mapping = {}
    for gene_pos, item_idx in enumerate(chromosome):
        slot_original_index = slots_sorted[gene_pos]
        mapping[item_idx] = slot_original_index
    return mapping

demo = list(range(min(10, TOP_N)))
random.shuffle(demo)
print("DEMO CHROMOSOME:", demo)

print("\nMakna (gene_pos -> slot_terdekat_ke-pos):")
for pos, item_idx in enumerate(demo[:10]):
    slot_idx = slots_sorted[pos]
    print(f"pos {pos}: item_idx {item_idx} ({item_codes[item_idx]}) -> slot {slot_idx} coord {slot_coords[slot_idx]} dist {slot_dist[slot_idx]}")
...
for gen in range(GENERATIONS):
    ...
    while len(new_pop) < POP_SIZE:
        parents, _, _, _ = roulette_wheel_select(population, fitnesses, n_select=2, debug_plot=False)
        p1, p2 = parents
...
plt.plot(best_history, label="Best Cost")
plt.plot(avg_history, label="Avg Cost")
plt.title("Konvergensi Algoritma Genetika")
plt.xlabel("Generasi")
plt.ylabel("Cost (Σ demand × jarak)")
plt.grid(True)
plt.legend()
plt.show()
...
for item_idx, slot_idx in mapping.items():
    rows.append({
        ...
        "CONTRIB_COST": item_demands[item_idx] * slot_dist[slot_idx]
    })
...
plt.title("Layout Gudang (isi sel = item_idx)")
...
plt.scatter([depot[1]], [depot[0]], s=220, marker="X")
